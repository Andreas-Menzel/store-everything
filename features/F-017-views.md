...
| People | dedicated page ([F-018](F-018-people.md) — deferred, joins when active) | rows are person *entities* (cover, name, counts, naming/merge/curation actions), not search results; "all files of person X" is an ordinary search (`persons` filter) and therefore view-able |
...
GET    /views          navigation entries: system views + own views + reserved entries for the
                       dedicated pages (browse, trash, duplicates, shared with me; people once
                       F-018 is active), per-user state applied, validity flags
