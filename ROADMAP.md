...
[F-004](features/F-004-document-text-extraction.md)'s split: everything lands in phase 2 except **segments feeding `text-v1` embeddings** ([F-004/FR-5](features/F-004-document-text-extraction.md)), which needs the `text-embed` extractor and closes with search in phase 3. Office text is *not* part of that split — [F-004/FR-3](features/F-004-document-text-extraction.md) asks for line-range **or section** anchors, and sections (heading path, sheet + row range, slide number) are what a `.docx` actually has; pure-python wheels produce them in phase 2. The `office-convert` LibreOffice step ([05](specs/05-extractor-contract.md)) refines those anchors into *page* anchors that match the office previews of [09](specs/09-previews.md), and travels with phase 3's heavy toolchain for that reason.
...
7. **[F-004](features/F-004-document-text-extraction.md) — document text**: `pdf-text` decision tree, `tesseract-ocr`, plain text/markdown/office/code — segments with page, line and section anchors, originals never modified *(the LibreOffice conversion that turns office sections into preview-matching pages is phase 3 — see the split above)*.
...
4. **[F-006](features/F-006-av-transcription-and-keyframes.md) — A/V**: Whisper-class transcription with timestamps; keyframes chained through the image pipeline — completes "video Y at 04:12". The same heavy-toolchain image work carries **audio cover art** ([F-028/FR-2](features/F-028-thumbnails-and-previews.md), phase 2's one staged part) and `office-convert`, whose LibreOffice `pdf` rendition chains into `pdf-text` to give office documents page anchors that match their previews.
