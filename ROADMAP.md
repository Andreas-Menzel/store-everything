...
[F-028](features/F-028-thumbnails-and-previews.md)'s split: everything lands in phase 2 except **audio cover art** — [F-028/FR-2](features/F-028-thumbnails-and-previews.md) names images, PDFs *and* audio as thumbnail sources, and reading an embedded picture out of an audio container belongs to the A/V toolchain that arrives with [F-006](features/F-006-av-transcription-and-keyframes.md) in phase 3 (there is no audio fixture before then either). So FR-2 stays unverified until phase 3 rather than being claimed on two of its three sources.

[F-003](features/F-003-tagging.md)'s split: everything lands in phase 2 except **tags as search facets** ([F-003/FR-7](features/F-003-tagging.md)), which needs [F-002](features/F-002-hybrid-search.md) and closes with search in phase 3.

[F-004](features/F-004-document-text-extraction.md)'s split: PDFs (with OCR), text, markdown and code land in phase 2; two parts wait for phase 3, each on a phase-3 component. **Segments feeding `text-v1` embeddings** ([F-004/FR-5](features/F-004-document-text-extraction.md)) need the `text-embed` extractor. **Office documents** ([F-004/FR-3](features/F-004-document-text-extraction.md)'s office half) need an anchor: the vocabulary is fixed at `page`/`time`/`line`/`sheet`/`region`/`whole` ([02 § Segment](specs/02-domain-model.md#segment)) and a `.docx` has none of them until something lays it out, so [05](specs/05-extractor-contract.md) routes office documents through `office-convert`'s LibreOffice `pdf` rendition — one conversion that serves both the page anchor and the office preview of [09](specs/09-previews.md), on the ~400 MB image that belongs with phase 3's heavy toolchain. Whether the vocabulary should gain a `section` kind instead is [Q64](OPEN-QUESTIONS.md).
...
7. **[F-027](features/F-027-web-application-shell.md) — web UI shell** (Vue 3 SPA — [ADR-0014](decisions/ADR-0014-vue-frontend-stack.md)), served by the API container at the same origin ([10 § topology](specs/10-deployment-and-operations.md#topology)): login, browse, upload, download — plus the **authenticated interactive API docs page** ([08](specs/08-api-principles.md)), which waits for login to exist because the schema endpoint is never public. The frame is its own feature because the parity bar in [F-025/FR-1](features/F-025-client-parity.md) needs "the web feature set" written down somewhere.
...
**Read first**
- **Features:** [F-001](features/F-001-upload-and-import.md) · [F-015](features/F-015-folders.md) · [F-027](features/F-027-web-application-shell.md) · [F-007](features/F-007-versioning.md) (write-path part) · [F-011](features/F-011-audit-trail.md) (log part) · [F-010](features/F-010-auto-sort-inbox.md) (v1 obligations only)
...
7. **[F-004](features/F-004-document-text-extraction.md) — document text**: `pdf-text` decision tree, `tesseract-ocr` (its own image), plain text/markdown/code — segments with page and line anchors, `searchable-pdf` rendition, originals never modified *(office documents wait for phase 3's `office-convert` — see the split above)*.
...
**Exit criteria:**
- Scanned PDF → OCR segments with page anchors; born-digital PDF → text-layer segments (no OCR run); text/markdown/code → line-anchored segments — verified against corpus fixtures. (Office documents are phase 3, with `office-convert` — see the split above.)
...
- F-003 green **except** [F-003/FR-7](features/F-003-tagging.md) (search facets), F-004 green **except** [F-004/FR-5](features/F-004-document-text-extraction.md) (the `text-embed` extractor) and FR-3's office half (`office-convert`), and [F-028](features/F-028-thumbnails-and-previews.md) green **except** [F-028/FR-2](features/F-028-thumbnails-and-previews.md)'s audio source — each names a phase-3 component and stays unverified rather than claimed early (see the splits above). [F-015/FR-9](features/F-015-folders.md) joins (folder tags), leaving only F-015's search part open. The phase-2 domain invariants carry markers — [02 § invariants](specs/02-domain-model.md#invariants) #3, #4, #5. Matrix green for everything claimed; phase tagged.
...
4. **[F-006](features/F-006-av-transcription-and-keyframes.md) — A/V**: Whisper-class transcription with timestamps; keyframes chained through the image pipeline — completes "video Y at 04:12". The same heavy-toolchain image work carries phase 2's two staged parts: **audio cover art** ([F-028/FR-2](features/F-028-thumbnails-and-previews.md)) and **`office-convert`**, whose LibreOffice `pdf` rendition chains into `pdf-text` so office documents get page anchors that match their previews ([F-004/FR-3](features/F-004-document-text-extraction.md)).
