...
- **Client caching:** because URLs are immutable per (file version, size), clients cache thumbnails indefinitely without revalidation ([13-mobile-clients](13-mobile-clients.md#caching-downloads-integrity)); a changed file is a new version and therefore a new URL. Whether a 512 px tier joins the fixed size set for high-density mobile grids is [Q42](../OPEN-QUESTIONS.md).
