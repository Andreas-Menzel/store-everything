...
**A tag's names are the tag's identity, and they live in one place.** The canonical name and every alias are rows of a single **name registry** keyed by the *folded* spelling (NFC, case-folded — the [names on disk](03-storage-and-portability.md#names-on-disk) rule reused, since "when are two spellings one name" is the same question; tag names additionally collapse internal whitespace). One keyed table rather than a name column plus an alias table, because the failure it prevents is the bad one: `bill` existing both as a tag and as a synonym of `invoice`, with the meaning decided by whichever query ran first. Two consequences fall out — resolving a label and completing a prefix are the *same lookup* (typing `automobile` can offer `car`, and the answer says which spelling matched), and a rename is a change of one row that every applied tag follows automatically. A rename does **not** leave the old name behind as an alias: a rename says the word was wrong, and an admin who wants it to keep resolving adds it back explicitly.

**Expansion reads a closure table** of *(ancestor, descendant, depth)* pairs, rebuilt from the edges whenever the taxonomy changes. Because this graph is a DAG rather than a tree, two tags can be connected by paths of different lengths; the pair is the key and `depth` is the **shortest** path, since expansion asks whether the row exists and the number only renders a breadcrumb. Merging two tags folds one into the other: applications move, the merged tag's names become aliases of the survivor, and where both tags sat on one file the stronger curation wins (`confirmed` > `manual` > `rejected`) — a merge must never silently delete curation. Hard deletion stays reserved for a typo nothing carries; anything applied is refused with `rejected` as the answer instead.

`FileTag` is the application of a tag to a file, carrying provenance. **Curation and machine claims are separate records**, which is what makes [invariant 4](#invariants) structural rather than vigilant: a user's `manual` / `confirmed` / `rejected` word is one row per *(file, tag)* — on the file, so it survives a new version of the same document — while an extractor's `auto` claim is derived data keyed by its run and stored with the other derived rows, per *version*, replaced wholesale by the next generation. The reprocessing path therefore has no reason to name the curation table, and does not. What the API exposes is one flat list per file with the provenance vocabulary below; where a curated word and a machine's claim meet, the curation decides.

A claim is keyed by *(version, tag, run)* rather than *(version, tag)*, because two extractors may both see a cat and a generation swap of one must not take the other's word away with it. A file's tags therefore resolve as: every curation row that is not a rejection, plus the **current** version's claims — from each extractor's **newest generation** — for tags nobody has ruled on. Older generations are kept rather than deleted ([ADR-0004](../decisions/ADR-0004-tag-provenance-and-reprocessing.md) retains one for rollback), so "replaced" is a property of the read; pruning them is [F-009](../features/F-009-reprocessing.md)'s business (Q63). Curation is what makes a tag survive: confirming keeps the claim's stamp visible alongside the `confirmed` state, and rejecting deletes the claims *and* leaves the negative record that stops any later generation re-adding the word — including one arriving through reuse of a byte-identical file's analysis.
...
**Usage counts are caller-scoped.** "How many files carry this tag", as autocomplete and facets report it, counts only what the caller can see — live files in workspaces they have access to. An instance-wide count would tell every member how many files somebody else has tagged `divorce`, and instance admin is not data access ([07](07-identity-permissions-sharing.md)). Trashed files are excluded inside the count, per [invariant 7](#invariants). The counts are computed per request today; if a large library makes that too slow, maintaining them incrementally is the known refinement (Q62) — and the same numbers are what search facets will need.
