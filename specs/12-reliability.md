...
|---|---|---|
| Extraction job ([04](04-ingestion-pipeline.md), [05](05-extractor-contract.md#job-lifecycle)) | operation row **plus** a 1:1 `extraction_run` sharing its id, both written in the transaction that created the file version ([ADR-0020](../decisions/ADR-0020-extractor-dispatch-and-wire-protocol.md)) | lease + fencing, with the extractor as the worker; results applied in one guarded transaction keyed by idempotency key. Routing is idempotent per *(version, extractor, generation)*, so re-detecting a version cannot double the work. The run mirrors the queue's decisions, including the ones no worker was alive to report — a claim that finds a job's attempts spent dead-letters it, and the next idle claim reconciles the run that still said `running` |
