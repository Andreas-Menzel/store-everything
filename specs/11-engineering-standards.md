...
- **The live-instance harness** (`server/tests/live_instance.py`) — uvicorn and a worker in their own threads, so a test can be a *client*: the extractor SDK is deliberately synchronous, and a synchronous client cannot speak to an ASGI application inside the test's own event loop. The one test that needs all three sides real at once uses it, and the conformance kit is exercised there as the command-line tool it is.
