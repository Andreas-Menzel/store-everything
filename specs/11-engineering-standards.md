...
- Tests carry the ids they verify as structured markers — `@pytest.mark.fr("F-002/FR-7")` in the core suite, `{ tags: ['@F-002/FR-7'] }` (Vitest) and `{ tag: ['@F-002/FR-7'] }` (Playwright) in the web suites ([ADR-0012](../decisions/ADR-0012-python-fastapi-core-stack.md), [ADR-0014](../decisions/ADR-0014-vue-frontend-stack.md)). One spelling with the `@` prefix across all three, because Playwright requires it. Domain invariants participate under stable ids (`02/INV-n` ↔ [02 § Invariants](02-domain-model.md#invariants) #n).
- **One report per test layer, merged into one matrix.** Each runner writes the same JSON document — `{layer, tests: {nodeid: {requirements, methods, outcome}}}` — and the matrix is built from all of them together (`core`, `web`, `web-e2e`). A requirement only a browser can verify therefore reaches exactly the same gate as one the core suite proves. A named report that does not exist is a hard error, never read as "uncovered": the matrix is built from a *full* run or not at all, and it prints which layer contributed how many marked tests so a matrix built from a filtered run is recognisable as one.
- The web runners reject a tag the configuration does not declare, and the declarations are generated from the feature files themselves — so a tag naming an FR that does not exist fails at collection, and the list can never go stale.
...
- **The matrix is authoritative from CI, not from a laptop.** A test that legitimately skips on the developer's platform — a case-collision test on a case-insensitive filesystem, say — reports its requirement as `skipped`, and `skipped` is not verified. A locally generated matrix therefore understates coverage; only the CI run, on the platform the product deploys to, is the answer.
- Needed from the **first feature PR**; specified here language-agnostically (marker convention + report contract), implemented in phase 0 against the [ADR-0012](../decisions/ADR-0012-python-fastapi-core-stack.md) toolchain and extended to the web suites at the phase-1 close ([Q59](../OPEN-QUESTIONS.md)).
...
- **Ground-truth corpus** — a versioned fixture set with a machine-readable manifest of each fixture's truth: PDFs with known phrases on known pages, images with known objects, audio/video with utterances at known timestamps — plus the adversarial set: unicode and case-colliding names, symlink layouts (both now with decided semantics to assert against — [ADR-0019](../decisions/ADR-0019-source-tree-semantics.md)), zip-slip archive entries, corrupt / zero-byte / oversized files. One corpus feeds FR tests, the golden-query benchmark, and the conformance kit. Fixtures must be redistributable, and each carries a manifest row (source, author, license, SHA-256, asserted truth) — sourcing, storage, and size budget: [ADR-0015](../decisions/ADR-0015-ground-truth-corpus-strategy.md).
- **Reference extractor** — deterministic and instant, shipped with the conformance kit in [`extractors/`](../extractors/README.md). Triple duty: E2E test double, executable example for third-party extractor authors, and the image the kit validates itself against. `SE_REFERENCE_MODE` also makes it misbehave on purpose (fail, fail permanently, stall), so the paths *around* a broken extractor are testable rather than hypothetical.
- **Conformance kit** — two suites, because the contract has two sides. **Protocol checks** drive a synthetic extractor and assert what the *core* owes: a refused manifest for the right reason, an identity that cannot be borrowed, one owner per exclusive output kind, a stale fencing token that cannot write, a disabled extractor that is told so. **Image checks** point at an extractor that is already running and assert what an *image* owes: it registers, its manifest is coherent, it claims promptly, it finishes, and it ignores what it never said it accepts. A check that cannot run here reports `skip` with its reason — a green tick nobody earned is worse than a gap.
- **The live-instance harness** (`server/tests/live_instance.py`, with the client-side helpers in `live_helpers.py`) — uvicorn and workers in their own threads, so a test can be a *client*: the extractor SDK is deliberately synchronous, and a synchronous client cannot speak to an ASGI application inside the test's own event loop. Every test that needs all three sides real at once uses it — the conformance kit as the command-line tool it is, the official extractors as the programs they are, and each phase's exit criterion as one ordered walk (`test_phase_one_walkthrough.py`, `test_phase_two_walkthrough.py`) because a suite of green units can all pass while the path between them is broken.
- **Tooling a test needs but a developer may not have** — an extractor whose tooling is a system package cannot be verified by a mock, and a requirement verified against a mock is not verified. Where the host may lack it, a small script runs the real thing from the image that ships it (`tools/tesseract-in-docker.sh` for OCR) and the test skips with that pointer as its reason. CI installs the package outright, so the skip is a local convenience and never a hole in the gate.
