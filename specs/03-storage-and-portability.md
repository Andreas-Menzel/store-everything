...
- **Progress is queryable while it runs** ([F-001/FR-5](../features/F-001-upload-and-import.md)): directories scanned, files seen, files registered, conflicts, skipped entries, and how much of the tree is still queued. Every re-scan adds what it concluded about files it already knew — changed, moved, trashed, restored — because those are the numbers an operator reads when something looks wrong: `trashed` climbing on a routine pass is how a half-mounted share announces itself. At 10 TB the initial pass runs for a long time, and "is it working?" has to be answerable without reading logs.
...
Detection is asymmetric on purpose, because its two errors are not equally bad. Missing a change costs an hour; concluding a deletion that did not happen costs an index. So four rules bound what a scan may conclude:

- **The marker gates the whole pass.** A root that does not carry the `.workspace/marker` planted at provisioning — or that carries another workspace's — is not this workspace's storage, most often because the mount is not mounted. The run registers nothing and reconciles nothing, reports why, and leaves the schedule armed ([F-001/FR-17](../features/F-001-upload-and-import.md)). An unmounted share and an emptied one look identical from a listing; the marker is what tells them apart. It is read **before the traversal and again before anything is reconciled**, including before each batch of reconciliation commits: a pass over ten terabytes lasts long enough for a share to leave in the middle of it, and every directory still queued then comes back *absent*.
- **Only an absence concludes a deletion.** A file is trashed only when the directory that should hold it was read — or confirmed gone — and did not mention its name. An entry the scan *saw* and refused is not an absence ([F-001/FR-18](../features/F-001-upload-and-import.md)).
- **A directory that could not be read concludes nothing about its contents**, and its whole subtree is excluded from that pass ([F-001/FR-16](../features/F-001-upload-and-import.md)).
- **A mount that went away is not an empty directory.** The marker speaks only for the root; a mount point *below* it that lost its mount is readable and empty — the directory that was always underneath, on the parent's filesystem — so nothing in the listing distinguishes it from one somebody emptied. The scan therefore records each directory's filesystem (`st_dev`) as it lists it, and an empty directory on a filesystem that is not the recorded one, with live files still indexed beneath it, is treated as unreadable ([F-001/FR-22](../features/F-001-upload-and-import.md)). The recorded filesystem is left as it was while that holds, so the subtree reconciles again by itself once the storage is back.

Even so, a wrong conclusion is recoverable rather than final: a trash entry is never purged before its deadline, and content reappearing at its own path reactivates the file it belonged to ([F-014/FR-10](../features/F-014-deletion-and-trash.md)), so a share that half-mounted heals itself on the next pass.
...
- **Restorability depends on who wrote** ("option b"): when the *app* mediates a change (API upload; later the laptop sync client turning a local save into a new-version call), it puts the old content into `versions/` before writing, and the predecessor stays fully restorable. A file edited *directly on disk* is overwritten before the app can snapshot it: re-scan records the new version, and the predecessor keeps all its previously extracted derived data — still **searchable**, but marked `restorable: false`. No copy-on-ingest, no doubled storage: only content that is actually superseded is ever stored twice.
- **The snapshot is a copy, not a move**, and the ordering is the reason. A move empties the file's own path until the new bytes are renamed in, and a scan interleaving with a multi-gigabyte upload would read that absent name as a deletion — it would trash the file mid-overwrite ([F-001/FR-18](../features/F-001-upload-and-import.md)), and a concurrent download would `404`. Copying keeps the path holding valid content at every instant, costs one extra read of the old file, and makes a crash mid-overwrite safe: the next scan finds the content changed and finds the predecessor's bytes already held. An overwrite whose snapshot does **not** match the version it supersedes is refused rather than applied ([F-001/FR-20](../features/F-001-upload-and-import.md)): the file changed on the storage, and losing that edit silently is worse than a `409`.
