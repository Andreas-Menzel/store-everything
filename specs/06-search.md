...
Tag filters **expand down the hierarchy by default** ([ADR-0006](../decisions/ADR-0006-hierarchical-tags-dag.md)): `tag:nature` matches files tagged with any descendant (`plant`, `tree`, …) via the precomputed closure table; `exact: true` restricts to the literal tag. Tags with status `suggested` are **excluded** from search, facets, and autocomplete until approved. Tag names are labels a person types and may contain spaces (`tax return`), so the query grammar has to quote them — `tag:"tax return"` — and a filter may equally carry a tag id, which needs no quoting at all; a client that has the id from autocomplete should send it.
...
### Date histogram & compact projection (timeline)
For timeline rendering ([F-017](../features/F-017-views.md) timeline layout, [F-020](../features/F-020-mobile-library.md)), a request may ask for a **date histogram** over a date-typed sort key at month/day granularity ([F-002/FR-19](../features/F-002-hybrid-search.md)): ordered `{period, count}` buckets over the entire matched set plus a trailing missing-key bucket, computed inside the query like facets — bucket counts over unreadable or trashed files are leaks. A listing may additionally opt into the **compact projection** ([F-002/FR-20](../features/F-002-hybrid-search.md)) — id, class, dimensions, sort-key value, duration, placeholder hash ([09](09-previews.md#thumbnails)) — same items, same order, same predicates as the full shape; it exists so grid clients can render whole periods from one small response.
