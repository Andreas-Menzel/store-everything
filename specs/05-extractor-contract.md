...
| `PUT /jobs/{id}/assets/{sha256}` | Stages one derived-asset payload by content hash — phase one of the two-phase result. The core verifies the bytes against the digest in the URL and refuses a mismatch, so the hash the envelope references always describes what was stored. Idempotent: re-staging the same bytes for the same job is a no-op, and staged files a result never referenced are collected by the janitor |
...
| `tags` | `{name, confidence}` list — **labels, not tag ids**. The core maps each one into its own vocabulary before storing it ([ADR-0006](../decisions/ADR-0006-hierarchical-tags-dag.md)): into an existing `active` tag if any spelling of one matches (canonical name or alias, which is where model drift is absorbed), otherwise into a new `suggested` tag quarantined until an admin approves it. A label the instance has already rejected — as a word, or by a user on this file — is dropped, and the count in the response says so. Stored as `auto` claims with the full provenance stamp (extractor, version, model, generation, confidence) |
| `embeddings` | `{space, segment_ref, vector}` list |
| `derived_assets` | previews/thumbnails, keyframes (with timestamps), transcript files, waveforms — staged by content hash, then referenced from the envelope as `{kind, name, content_hash, media_type, params?, rendition_kind?}`. The **extractor names the file** and the core places it under the source version's hash ([09 § storage](09-previews.md#storage)); a name that is not a name — a path, a hidden file — is refused rather than sanitised |
...
The line between tolerating and refusing runs between *fields* and *values*. An unknown field is kept and ignored, because a newer extractor must keep working against an older core. An unknown metadata **type** or anchor **kind** is refused with a field-level problem, because storing it correctly is impossible and accepting it would mean dropping data silently — and the envelope is then refused **whole**: no half of a result is ever applied.
...
|---|---|---|---|
| `basic-metadata` | `*/*` | metadata | **exiftool + ffprobe** (Artistic/GPL dual; LGPL build): size, times, EXIF/XMP/IPTC, media info (duration, codecs, dimensions); emits `detected_media_type` when content contradicts the extension — the correction path for extension-guessed types ([04 § identification](04-ingestion-pipeline.md#2-identification)). **Phase 2 ships the libvips/PDFium half of this** — EXIF dates, camera, GPS, ISO, orientation; a PDF's title, author and date — because both libraries are already in the image, both read headers without decoding, and neither needs a Perl runtime. exiftool and ffprobe arrive with the A/V work in phase 3, together with the formats (containers, XMP/IPTC sidecars, maker notes) that actually require them |
| `pdf-text` | PDF | text_segments (page anchors), metadata | **PyMuPDF** (AGPL-3.0 — matches the project license): words with bounding boxes per page in ~0.1 s/doc; per-page decision tree (text present → garble check via U+FFFD/character-plausibility → image-only page ⇒ `needs_ocr` + `ocr_pages`) |
| `pdf-pages` | PDF | derived_assets (page images) | **pypdfium2** (Apache/BSD): renders page images — page 1 with the thumbnail, the rest on demand per [09](09-previews.md#previews). Split out of `pdf-text` when both shipped: a person waiting on page 40 of a document they are reading must not queue behind a 300-page text extraction, and an on-demand render needs its own cost class and interactive priority ([04 § prioritization](04-ingestion-pipeline.md)). Two renderers is the deliberate cost — text wants PyMuPDF's word boxes, pixels want PDFium's speed |
| `tesseract-ocr` | images, keyframes; PDFs `when needs_ocr` | text_segments (page/region anchors + confidence), renditions | **Tesseract 5.5** (Apache-2.0), `deu+eng+osd` tessdata_fast baked into the image (no network, ~16 MB); word boxes + confidence via TSV at ~300 dpi, `OMP_THREAD_LIMIT=1` (parallelism across jobs); the `searchable-pdf` rendition is assembled from the **same** recognition pass — Tesseract writes TSV and PDF from one invocation, so the rendition costs nothing extra. **Its own image** (`extractors/Dockerfile.ocr`, on Debian trixie for the 5.5 packages): the split the shared image's comment anticipated, since the engine and its models are the first tooling that is not a wheel. **Phase 2 accepts PDFs only** — image OCR is [F-005/FR-3](../features/F-005-image-analysis.md), and it arrives with the rest of image analysis in phase 3 rather than putting every photograph in the library through OCR before anything can use the result. (Rejected: Surya — RAIL-M weight license + ~9 s/page CPU; PaddlePaddle runtime — 195 MB wheel. A RapidOCR/PP-OCRv5 image, Apache-2.0 end to end, is a clean optional extractor later.) |
| `text-plain` | text, markdown, code, CSV, JSON/XML/YAML/TOML | text_segments, metadata (language, encoding) | stdlib + charset-normalizer (MIT): line-range anchors, and the encoding *detected* rather than assumed — a file that decodes to nothing legible is a reported failure instead of a table full of mojibake ([F-004/FR-6](../features/F-004-document-text-extraction.md)); per-document language detection via **lingua** (Apache-2.0) → well-known `language` key (drives language-aware FTS — [F-004/FR-4](../features/F-004-document-text-extraction.md)). Spreadsheets go with the office formats below: `sheet` anchors are in the vocabulary, but one extractor per anchor story is the simpler split |
| `office-convert` | office documents (docx/xlsx/pptx/odt/rtf, …) | renditions (`pdf`) | **LibreOffice headless** (MPL-2.0): converts once to a `pdf` rendition that **chains into `pdf-text`** — page anchors that match the page previews users actually see, and the office preview path of [09](09-previews.md#previews), from one conversion. This is *the* office text path, not an optimization of one: the anchor vocabulary is fixed ([02 § Segment](02-domain-model.md#segment)) and a `.docx` has no page until something lays it out, so reading its text without converting it would produce segments with nowhere to point ([Q64](../OPEN-QUESTIONS.md)). Phase 3, with the rest of the heavy toolchain |
...
| `preview-gen` | images, PDFs (audio with cover art in phase 3) | derived_assets (thumbnails, placeholder hashes, image previews) | **libvips** (LGPL, HEIC via libheif) for images; **pypdfium2** (Apache/BSD) renders PDF page 1 for its thumbnail — its own renderer so P1 thumbnails never wait behind the P2 `pdf-text` queue; ffmpeg extracts embedded audio cover art (phase 3, which is when `audio/*` joins its `accepts`); thumbhash-class placeholders implemented in-house. `accepts` names the types that *have* a picture rather than `*/*`: a text file has no thumbnail source, and a job that could only decline would leave a run on every note in the library where [F-028/FR-3](../features/F-028-thumbnails-and-previews.md) asks for a typed "no thumbnail". Type-specific preview assets live with their type's extractors ([09-previews.md](09-previews.md)) |
