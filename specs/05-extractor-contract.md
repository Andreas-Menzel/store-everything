...
| `preview-gen` | `*/*` | derived_assets (thumbnails, placeholder hashes, image previews) | **libvips** (LGPL, HEIC via libheif) for images; **pypdfium2** (Apache/BSD) renders PDF page 1 for its thumbnail — its own renderer so P1 thumbnails never wait behind the P2 `pdf-text` queue; ffmpeg extracts embedded audio cover art; thumbhash-class placeholders implemented in-house. Type-specific preview assets live with their type's extractors ([09-previews.md](09-previews.md)) |
