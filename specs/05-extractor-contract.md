...
| `text-plain` | text, markdown, code, CSV, office documents | text_segments, metadata (language) | stdlib + charset-normalizer (MIT): line-range anchors, encoding detected rather than assumed; office documents via **python-docx / openpyxl / python-pptx / odfpy** (MIT/MIT/MIT/Apache-2.0) with section anchors — heading path, sheet + row range, slide number — which is the anchor [F-004/FR-3](../features/F-004-document-text-extraction.md) asks for and the one those formats have before layout; per-document language detection via **lingua** (Apache-2.0) → well-known `language` key (drives language-aware FTS — [F-004/FR-4](../features/F-004-document-text-extraction.md)) |
| `office-convert` | office documents (docx/pptx/odt/rtf, …) | renditions (`pdf`) | **LibreOffice headless** (MPL-2.0): converts once to a `pdf` rendition that **chains into `pdf-text`** — page anchors that match the page previews users actually see, and the office preview path of [09](09-previews.md#previews), from one conversion. A refinement, not a prerequisite: `text-plain` already yields office *section* anchors without a 400 MB image, so this arrives with the rest of phase 3's heavy toolchain |
